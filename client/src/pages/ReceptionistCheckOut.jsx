...
import { Search as SearchIcon, Refresh as RefreshIcon, Check as CheckIcon, Close as CloseIcon, Person as PersonIcon, Room as RoomIcon, Key as KeyIcon, EventNote as NoteIcon, CalendarToday as CalendarIcon, Sync as SyncIcon, PersonOutline as ProfileIcon, PowerSettingsNew as LogoutIcon } from '@mui/icons-material';
...
const ReceptionistCheckOut = () => {
    ...
    const fetchCheckOuts = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.message || 'Error al cargar check-outs')
            }
    ...
    const filteredCheckOuts = checkOuts.filter(checkOut => {
        const matchesSearch = checkOut.guestName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                              ...
                              checkOut.guestEmail.toLowerCase().includes(searchTerm.toLowerCase())

        const matchesFilter = filterStatus === 'all' ||
                            (filterStatus === 'today' && checkOut.checkOutDate === today) ||
                            (filterStatus === 'future' && checkOut.checkOutDate > today) ||
                            (filterStatus === 'past' && checkOut.checkOutDate < today)
    ...
    const handleCheckOut = async (reservation) => {
        try {
            ...
            if (response.data.success) {
                setCheckOuts(prev =>
                    prev.filter(r => r.bookingId !== reservation.bookingId) // Eliminar la reserva del check-out
                )
                showSnackbar(response.data.message || 'Check-out procesado correctamente', 'success')
            } else {
                throw new Error(response.data.message || 'Error al procesar check-out')
            }

        } catch (error) {
            console.error('Error en check-out:', error)
            showSnackbar(error.response?.data?.message || 'Error al procesar check-out', 'error')
        }
    }
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', flex: 1, paddingLeft: '270px' }}>
                                <SectionTitle subtitle="Gestiona el proceso de salida de huéspedes"> Check-out </SectionTitle>
                            </Box>

                           <LoadingButton variant="contained" startIcon={<RefreshIcon />} onClick={fetchCheckOuts} loading={loading} sx={{ backgroundColor: 'primary.main', '&:hover': { backgroundColor: 'primary.dark' } }}>
                                Actualizar
                            </LoadingButton>
                        </Box>
                            ...
                            <Grid container spacing={2} alignItems="center">
                                ...
                                <Grid item xs={12} sm={12} md={4}>
                                    <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                                        <Chip label={`Total: ${filteredCheckOuts.length}`} color="primary" variant="outlined" />
                                        <Chip label={`Hoy: ${filteredCheckOuts.filter(r => r.checkOutDate === today).length}`} color="warning" variant="outlined" />
                                    </Box>
                                </Grid>
                            </Grid>
                        </Box>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {loading ? (
                                                ...
                                                <TableCell colSpan={7} align="center" sx={{ py: 4 }}>
                                                    <Box sx={{ textAlign: 'center' }}>
                                                        ...
                                                        <Typography variant="body2" color="text.secondary">
                                                            {searchTerm ? 'Intenta con otros términos de búsqueda' : 'No hay reservas pendientes de check-out'}
                                                        </Typography>
                                                    </Box>
                                                </TableCell>
                                            </TableRow>
                                            ...
                                            filteredCheckOuts.map((reservation, index) => (
                                                <TableRow key={reservation.id} hover sx={{ '&:hover': { backgroundColor: 'rgba(41, 163, 116, 0.04)' } }}>
                                                    ...
                                                    <TableCell>
                                                        <Box>
                                                            ...
                                                            <Typography variant="caption" color="text.secondary">
                                                                {reservation.checkOutTime || '12:00'}
                                                            </Typography>
                                                        </Box>
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Chip
                                                            label={reservation.canCheckOut ? "Disponible" : reservation.checkOutDate > new Date().toISOString().split('T')[0] ? "Futuro" : "No disponible"}
                                                            color={reservation.canCheckOut ? "warning" : reservation.checkOutDate > new Date().toISOString().split('T')[0] ? "info" : "default"}
                                                            size="small"
                                                        />
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Box sx={{ display: 'flex', gap: 1 }}>
                                                            ...
                                                            {reservation.canCheckOut ? (
                                                                <Tooltip title="Procesar Check-out">
                                                                    <IconButton size="small" onClick={() => handleCheckOut(reservation)} sx={{ backgroundColor: 'warning.main', color: 'white', '&:hover': { backgroundColor: 'warning.dark' } }}>
                                                                        <CheckIcon fontSize="small" />
                                                                    </IconButton>
                                                                </Tooltip>

                                                            ) : (
                                                                <Tooltip title={reservation.checkOutDate > new Date().toISOString().split('T')[0] ? "Check-out futuro" : "Check-out no disponible"}>
                        ...
                        <Box sx={{ p: 3 }}>
                            <Grid container spacing={3}>
                                ...
                                <Grid item xs={12} sm={6}>
                                    ...
                                    <Typography variant="body2" color="text.secondary"> {selectedReservation.checkInTime}
                                    </Typography>
                                </Grid>
                                ...
                                <Grid item xs={12} sm={6}>
                                    ...
                                    <Typography variant="body2" color="text.secondary"> {selectedReservation.checkOutTime} </Typography>
                                </Grid>

                                <Grid item xs={12} sm={6}>
                                    <Typography variant="subtitle2" gutterBottom>Huéspedes</Typography>
                                ...
                                <Grid item xs={12} sm={6}>
                                    <Typography variant="subtitle2" gutterBottom>Monto Total</Typography>
                                    <Typography variant="body1" fontWeight="bold" color="primary.main"> ${selectedReservation.totalAmount?.toFixed(2)} </Typography>
                                </Grid>
                            </Grid>
