...
import { ArrowBack as ArrowBackIcon, Security as PermissionIcon, Gavel as SystemIcon, Add as AddIcon, Assignment as AssignIcon, Person as PersonIcon } from '@mui/icons-material'
...
const PermissionManagement = () => {
    ...
    const [confirmDialog, setConfirmDialog] = useState({ open: false, permission: null, message: '' })
    ...
    const handleSavePermission = async (permissionData) => {
        try {
            ...
            showSnackbar(`Error al ${editingPermission ? 'actualizar' : 'crear'} el permiso: ${err.response?.data?.message || err.message}`, 'error')
        }
    }
    ...
    const confirmDeletePermission = (permission) => {
        setConfirmDialog({
            open: true,
            permission,
            message: `¿Está seguro de que desea eliminar el permiso "${permission.name}"? Esta acción no se puede deshacer.`
        })
    }
    ...
    const handleDeletePermission = async () => {
        ...
        try {
            ...
            showSnackbar(`Error al eliminar el permiso: ${err.response?.data?.message || err.message}`, 'error')
        }
    }
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
                ...
                <Container maxWidth="xl">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                                <Typography variant="h4" fontWeight="bold" color="primary.main"> Gestión de Permisos </Typography>
                            </Box>
                        </Box>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }}>
                        <PermissionsTable
                            ...
                            loading={loading}
                            onEditPermission={handleOpenDialog}
                            onDeletePermission={confirmDeletePermission}
                        />
                    </motion.div>
                </Container>
            </Box>
            ...
            <ConfirmActionDialog
                ...
                userName={confirmDialog.permission?.name || ''}
                actionType="delete"
                loading={loading}
                entityType="permiso"
            />
